...
import DishPresentationForm from "./DishPresentationForm";  // form component
import DishPresentationFilters from "./DishPresentationFilters"; // filter component
...
const DishPresentationList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_DISH_PRESENTATION, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_DISH_PRESENTATION, API_PATH_DISH_PRESENTATION, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_DISH_PRESENTATION, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_DISH_PRESENTATION))
        ModalContainer( DishPresentationForm )
    }

    const onEdit = data => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_DISH_PRESENTATION, data))
        ModalContainer( DishPresentationForm,{ data })
    }

    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Eliminar el tipo de presentación`,
        storePath:STORE_PATHS_DISH_PRESENTATION,
...
export default DishPresentationList