...
import DrinkCategoryForm from "./DrinkCategoryForm";  // form component
import DrinkCategoryFilters from "./DrinkCategoryFilters"; // filter component
...
// paths
import {API_PATH_DRINK_CATEGORY} from "../../../connection/apiPaths"
...
const DrinkCategoryList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_DRINK_CATEGORY, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_DRINK_CATEGORY, API_PATH_DRINK_CATEGORY, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_DRINK_CATEGORY, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_DRINK_CATEGORY))
        ModalContainer( DrinkCategoryForm )
    }

    const onEdit = data => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_DRINK_CATEGORY, data))
        ModalContainer( DrinkCategoryForm,{ data })
    }

    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Eliminar la categoría de la bebida`,
        storePath:STORE_PATHS_DRINK_CATEGORY,
    ...
    return <Container>
        <Card>
            <CardTitle>
                Lista de categorías de la bebida
