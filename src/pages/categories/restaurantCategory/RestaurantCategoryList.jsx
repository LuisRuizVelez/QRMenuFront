...
import RestaurantCategoryForm from "./RestaurantCategoryForm";  // form component
import RestaurantCategoryFilters from "./RestaurantCategoryFilters"; // filter component
...
const RestaurantCategoryList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_RESTAURANT_CATEGORY, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_RESTAURANT_CATEGORY, API_PATH_RESTAURANT_CATEGORY, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_RESTAURANT_CATEGORY, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_RESTAURANT_CATEGORY))
        ModalContainer( RestaurantCategoryForm )
    }

    const onEdit = data => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_RESTAURANT_CATEGORY, data))
        ModalContainer( RestaurantCategoryForm,{ data })
    }

    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Eliminar categoría`,
        storePath:STORE_PATHS_RESTAURANT_CATEGORY,
    ...
    return <Container>
        <Card>
            <CardTitle>
                Listado de categorías de restaurantes
