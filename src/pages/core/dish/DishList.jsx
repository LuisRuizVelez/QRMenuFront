...
import DishForm from "./DishForm";  // form component
import DishFilters from "./DishFilters"; // filter component
...
const DishList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_CORE_DISH, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_CORE_DISH, API_PATH_CORE_DISH, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_CORE_DISH, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_CORE_DISH))
        ModalContainer( DishForm )
    }
    ...
    const onDelete = ({id}) => ConfirmDelete({
        ...
        storePath:STORE_PATHS_CORE_DISH,
