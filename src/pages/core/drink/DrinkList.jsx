...
import DrinkForm from "./DrinkForm";  // form component
import DrinkFilters from "./DrinkFilters"; // filter component
...
// paths
import {API_PATH_CORE_DRINK} from "../../../connection/apiPaths"
...
const DrinkList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_CORE_DRINK, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_CORE_DRINK, API_PATH_CORE_DRINK, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_CORE_DRINK, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_CORE_DRINK))
        ModalContainer( DrinkForm )
    }
    ...
    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Eliminar la bebida`,
        storePath:STORE_PATHS_CORE_DRINK,
        servicePath: API_PATH_CORE_DRINK,
    })
