...
import MenuForm from "./MenuForm";  // form component
import MenuFilters from "./MenuFilters"; // filter component
...
// paths
import {API_PATH_CORE_MENU} from "../../../connection/apiPaths"
...
const MenuList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_CORE_MENU, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_CORE_MENU, API_PATH_CORE_MENU, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_CORE_MENU, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_CORE_MENU))
        ModalContainer( MenuForm )
    }

    const onEdit = data => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_CORE_MENU, data))
        ModalContainer( MenuForm,{ data })
    }

    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Eliminar el menu`,
        storePath:STORE_PATHS_CORE_MENU,
