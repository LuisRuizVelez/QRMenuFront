import React, {useEffect} from "react";
import {useDispatch, useSelector} from "react-redux";
import {navigationAddState, navigationRemoveState, getStateByPath} from "../../../store/navigation/navigationThunks";


// components
import Loader from "../../../ui/Loader";
import RestaurantList from "./RestaurantList"; // list component


// paths
import {STORE_PATHS_CORE_RESTAURANT} from "../../../store/StorePaths";



const Restaurant = () => {
    const dispatch = useDispatch();
    const componentState = useSelector(state => getStateByPath(state, STORE_PATHS_CORE_RESTAURANT));

    const {loading} = componentState?.data || false;


    // Add navigation state when component mounts
    useEffect(() => {
        dispatch( navigationAddState(STORE_PATHS_CORE_RESTAURANT) );

        return () => {
            dispatch( navigationRemoveState(STORE_PATHS_CORE_RESTAURANT) );
        };
    }, [dispatch]);





    return <>
        { loading && Loader() }
        <RestaurantList />
    </>
}

export default Restaurant