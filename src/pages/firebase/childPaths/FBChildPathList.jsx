...
import FBChildPathForm from "./FBChildPathForm";  // form component
import FBChildPathFilters from "./FBChildPathFilters"; // filter component
...
// paths
import {API_PATH_FB_CHILD_PATH} from "../../../connection/apiPaths"
import {STORE_PATHS_FB_CHILD_PATH} from "../../../store/StorePaths";



const FBChildPathList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_FB_CHILD_PATH, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_FB_CHILD_PATH, API_PATH_FB_CHILD_PATH, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_FB_CHILD_PATH, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_FB_CHILD_PATH))
        ModalContainer( FBChildPathForm )
    }

    const onEdit = data => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_FB_CHILD_PATH, data))
        ModalContainer( FBChildPathForm,{ data })
    }

    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Eliminar la ruta`,
        storePath:STORE_PATHS_FB_CHILD_PATH,
        servicePath: API_PATH_FB_CHILD_PATH,
    })
    ...
    return <Container>
        <Card>
            <CardTitle>
                Paths de los componentes en Firebase
...
export default FBChildPathList;