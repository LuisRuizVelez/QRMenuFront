...
import RoleForm from "./RoleForm";  // form component
import RoleFilters from "./RoleFilters"; // filter component
...
const RoleList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_ROLE, {
            item:{ code : null}
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_ROLE, API_PATH_ROLE, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_ROLE, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_ROLE))
        ModalContainer( RoleForm )
    }

    const onEdit = data => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_ROLE, data))
        ModalContainer( RoleForm,{ data })
    }

    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Elimar el Role`,
        storePath:STORE_PATHS_ROLE,
