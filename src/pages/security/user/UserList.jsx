...
import UserForm from "./UserForm";  // form component
import UserFilters from "./UserFilters"; // filter component
...
const UserList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(navigationSetFilter(STORE_PATHS_USER, {
            item:{ enabled : true }
        }))
    }, [dispatch, filter]);
    ...
    useEffect(() => {
        ...
        navigationActions.search(STORE_PATHS_USER, API_PATH_USER, page, sizePerPage, filter)
    }, [page, sizePerPage, filter]);


    const handleTableChange = (type, {page, sizePerPage}) => dispatch( navigationSetTableParams(STORE_PATHS_USER, page, sizePerPage, totalSize) )


    const onCreate = () => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_USER))
        ModalContainer( UserForm )
    }

    const onEdit = data => {
        dispatch(navigationSetSelectedItem(STORE_PATHS_USER, data))
        ModalContainer( UserForm,{ data })
    }

    const onDelete = ({id}) => ConfirmDelete({
        id,
        title: `Eliminar Usuario`,
        storePath:STORE_PATHS_USER,
        servicePath: API_PATH_USER,
    })
